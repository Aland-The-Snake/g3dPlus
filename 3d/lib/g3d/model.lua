...
local loadJsonModel = require(g3d.path .. ".jsonModel")
...
-- Modified newModel function to support JSON files
local function newModel(verts, texture, translation, rotation, scale)
    local self = setmetatable({}, model)

    -- if verts is a string, determine if it's OBJ or JSON
    if type(verts) == "string" then
        if verts:match("%.json$") then
            verts = loadJsonModel(verts)
        else
